...
def make_pie(entered_value):  
    ...
    if entered_value == "ALL":  
        ...
        df_2 = spacex_df[spacex_df['Launch Site'] == entered_value] 

        df_final = df_2.groupby(by=['class']).size().to_frame(name='count').reset_index()  

        fig = px.pie(df_final, values='count', names='class', title = 'Total Success Launches for site ' + entered_value)  
...
def make_scatter(ddl_value, slider_value): 
    ...
    if ddl_value == "ALL": 
        ...
        restricted_df = spacex_df[spacex_df['Launch Site'] == ddl_value] 

        mask = (restricted_df['Payload Mass (kg)'] > low) & (restricted_df['Payload Mass (kg)'] < high) 

        fig = px.scatter( 

        restricted_df[mask], x="Payload Mass (kg)", y="class",  
